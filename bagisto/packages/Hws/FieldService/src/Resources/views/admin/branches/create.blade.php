    ...
    <div class="content">
        ...
        <div class="page-content">
            ...
            <div style="background: #fff; border: 1px solid #e2e8f0; border-radius: 8px; padding: 24px; margin-bottom: 24px;">
                <form method="POST" action="{{ route('hws.admin.branches.store') }}">
                    @csrf
                    ...
                    <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(220px, 1fr)); gap: 16px; margin-bottom: 20px;">
                        <div>
                            ...
                            <input type="text" name="code" value="{{ old('code') }}" required placeholder="e.g. DEL-01" style="width: 100%; border: 1px solid #cbd5e1; border-radius: 8px; padding: 8px 12px; font-size: 14px; box-sizing: border-box;"/>
                        </div>
                        ...
                        <div>
                            ...
                            <input type="text" name="name" value="{{ old('name') }}" required placeholder="e.g. Delhi Central Branch" style="width: 100%; border: 1px solid #cbd5e1; border-radius: 8px; padding: 8px 12px; font-size: 14px; box-sizing: border-box;"/>
                        </div>

                        <div>
                            <label style="display: block; font-size: 12px; font-weight: 700; color: #475569; margin-bottom: 6px;">Phone Number</label>
                            <input type="text" name="phone" value="{{ old('phone') }}" placeholder="+91-XXXXXXXXXX" style="width: 100%; border: 1px solid #cbd5e1; border-radius: 8px; padding: 8px 12px; font-size: 14px; box-sizing: border-box;"/>
                        </div>

                        <div>
                            <label style="display: block; font-size: 12px; font-weight: 700; color: #475569; margin-bottom: 6px;">Branch Email</label>
                            <input type="email" name="email" value="{{ old('email') }}" placeholder="branch@domain.com" style="width: 100%; border: 1px solid #cbd5e1; border-radius: 8px; padding: 8px 12px; font-size: 14px; box-sizing: border-box;"/>
                        </div>

                        <div>
                            <label style="display: block; font-size: 12px; font-weight: 700; color: #475569; margin-bottom: 6px;">GSTIN</label>
                            <input type="text" name="gstin" value="{{ old('gstin') }}" placeholder="05AAAAA0000A1Z5" style="width: 100%; border: 1px solid #cbd5e1; border-radius: 8px; padding: 8px 12px; font-size: 14px; box-sizing: border-box;"/>
                        </div>
                    </div>
                    ...
                    <div style="margin-bottom: 16px;">
                        ...
                        <textarea name="address" rows="2" placeholder="Building, Street, Industrial Area..." style="width: 100%; border: 1px solid #cbd5e1; border-radius: 8px; padding: 8px 12px; font-size: 14px; resize: vertical; box-sizing: border-box;">{{ old('address') }}</textarea>
                    </div>

                    <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 16px; margin-bottom: 20px;">
                        <div>
                            <label style="display: block; font-size: 12px; font-weight: 700; color: #475569; margin-bottom: 6px;">City</label>
                            <input type="text" name="city" value="{{ old('city') }}" placeholder="e.g. Dehradun" style="width: 100%; border: 1px solid #cbd5e1; border-radius: 8px; padding: 8px 12px; font-size: 14px; box-sizing: border-box;"/>
                        </div>
                        ...
                        <div>
                            ...
                            <select name="state" required style="width: 100%; border: 1px solid #cbd5e1; border-radius: 8px; padding: 8px 12px; font-size: 14px; box-sizing: border-box; background: #fff;">
                                ...
                                @foreach($states as $st)
                                    <option value="{{ $st->name }}" {{ old('state') == $st->name || old('state') == $st->code ? 'selected' : '' }}>
                                        {{ $st->name }} ({{ $st->code }})
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <input type="text" name="pincode" value="{{ old('pincode') }}" placeholder="e.g. 248001" style="width: 100%; border: 1px solid #cbd5e1; border-radius: 8px; padding: 8px 12px; font-size: 14px; box-sizing: border-box;"/>
                        </div>
                    </div>
                    ...
                    <div style="display: flex; gap: 24px; align-items: center; margin-bottom: 16px;">
                        <label style="display: flex; align-items: center; gap: 8px; font-weight: 600; color: #0f172a; cursor: pointer; font-size: 13px;">
                            <input type="checkbox" name="is_head_office" value="1" {{ old('is_head_office') ? 'checked' : '' }}>
